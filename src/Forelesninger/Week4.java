...
public class Week4 {
    ...
    public static <T extends Comparable<? super T>> boolean isGreaterThan(T a, T b) {
        //jeg vil at dette skal være både for integer, string, float og alt mulig.
        System.out.println("a="+ a + ", b=" + b);

        //a.compareTo() fungerer for string
        //if (a > b) --> denne funker ikke for string
        if (a.compareTo(b) > 0) {
            return true;
        }
        else {
    ...
    public static void quickSort(int[] values, int left, int right) {
        if (left >= right) {
            return; //da har vi bare returnert hvis vi ikke har noe intervall lenger.
        }
        ...
        if(left >= right){
            return; //dette er stoppekriterie
        }

        //først velge en pertisjonsverdi
        int partition_value = 7;

        //den peker der det sorterte tallet er int position
        int position = partitionList(values, partition_value);

        //gjenta for høyre og venstre subliste
        quicksort(values, position+1, right);
        quicksort(values, left, position-1);
        //startet med ett funksjonskall, og nedte opp med to

        //Den må ha et sted hvor den slutter
