...
public class Week2 {
    ...
    public static int maximum(int[] values, int begin, int end) { //det er begynnelsen og slutten
        //Parametersjekking
        if (begin < 0 || end >values.length) {
            throw new ArrayIndexOutOfBoundsException("Feil parameterverdier"); //kan kaste forskjellige exceptions, butde skille på de tre forskjellige feilene
        }
        else if (begin >= end) {
        ...
        //NOTE: this for loop loops over all elements in [begin+1, end-1]
...
/**
 * lønner seg å gjøre ting i mindre funksjoner
 *
 * Values er verdiene vi skal jobbe på
 * begin og n er hviljen intervall jeg skal søke gjennom for å finne maximum
 **/