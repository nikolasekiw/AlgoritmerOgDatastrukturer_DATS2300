...
public class Week3 {
    ...
    public static int findMax(int[] values, int left, int right) {
        ...
        for (int i=left+1; i<=right; ++i) { //+1 fordi den allerede er initlialisert over
    ...
    public static int binarySearch(int[] values, int value) {
        ...
        while (left <= right) {
            ...
            else if (value > values[mid]) { //v peker opp til midten + 1 fordi da søker vi i siste intervalet
                //Search right of mid
                left = mid + 1;
            }
            if (value == values[mid]) {
    ...
    /**
     * Finner max på mindre og mindre intervall.
     * Dette er selectionsort: velger et element og putter det først.
     * Bruker funksjonen mindMin istedenfor findMax for å sortere stigende
     * og forandrer bedre på det, alt annet er likt
     */
